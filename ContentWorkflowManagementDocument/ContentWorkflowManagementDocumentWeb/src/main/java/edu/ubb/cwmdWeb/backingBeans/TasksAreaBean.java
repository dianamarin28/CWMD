...
@ManagedBean
@ViewScoped
public class TasksAreaBean implements Serializable{

	private static final long serialVersionUID = 7979129834247710500L;
	...
	public void initBean() {
		...
		if (userRole.equals("administrator")) {
			...
			activeFlowDTOs = activeFlowBeanInterface.getActiveByStep(userDTO.getUserId());
		}
	...
	public void approveVersionDTO(VersionDTO versionDto)
	{
		System.out.println("approved");
		this.approved++;
		if(this.approved == this.versionDTOs.size())
		{
			System.out.println("Approved flow step");
			activeFlowBeanInterface.approveActiveFlow(selectedActiveFlow);
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "Approved", "Approved flow " + selectedActiveFlow.getName());
			FacesContext.getCurrentInstance().addMessage(null, msg);
		}
		else
		{
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "Approved", "Approved document " + versionDto.getFileName());
			FacesContext.getCurrentInstance().addMessage(null, msg);
		}
	}
	
	public void rejectVersionDTO(VersionDTO versionDto)
	{
		System.out.println("rejected");
		activeFlowBeanInterface.rejectActiveFlow(selectedActiveFlow);
		FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "Rejected", "Rejected flow " + selectedActiveFlow.getName());
		FacesContext.getCurrentInstance().addMessage(null, msg);
	}
